...
// Scans an archive once, recording each object's stored hash and the
// offset of its record start (the same offset convention ObjectPut's
// callers already track manually, e.g. tree_off/commit2_off in earlier
// probes - this is what replaces having to track that by hand).
...
// Linear lookup (no hash table yet - see probe README for why this is
// still a real capability gain over what existed before: nothing could
// look up "the object with this hash" at all until now).
...
// --- build the same small object graph as probe 10/11: 2 blobs, 1 tree ---
U8 archive[4096];
I64 alen = 16;
HgsWriteHeader(archive, 1, 3);

U8 blob1[5]; blob1[0]='h';blob1[1]='e';blob1[2]='l';blob1[3]='l';blob1[4]='o';
U8 blob2[5]; blob2[0]='w';blob2[1]='o';blob2[2]='r';blob2[3]='l';blob2[4]='d';
U8 blob1_hash[64], blob2_hash[64];
U8 tg1[6]; tg1[0]=OBJ_BLOB;tg1[1]='h';tg1[2]='e';tg1[3]='l';tg1[4]='l';tg1[5]='o';
B2Hash512Any(tg1,6,blob1_hash);
U8 tg2[6]; tg2[0]=OBJ_BLOB;tg2[1]='w';tg2[2]='o';tg2[3]='r';tg2[4]='l';tg2[5]='d';
B2Hash512Any(tg2,6,blob2_hash);
ObjectPut(archive,&alen,OBJ_BLOB,blob1,5);
ObjectPut(archive,&alen,OBJ_BLOB,blob2,5);

U8 tree_content[512];
I64 tlen=0;
PutU32LE(tree_content,0,2); tlen=4;
U8 na[5]; na[0]='a';na[1]='.';na[2]='t';na[3]='x';na[4]='t';
U8 nb[5]; nb[0]='b';nb[1]='.';nb[2]='t';nb[3]='x';nb[4]='t';
TreeEncodeEntry(tree_content,&tlen,na,5,OBJ_BLOB,blob1_hash);
TreeEncodeEntry(tree_content,&tlen,nb,5,OBJ_BLOB,blob2_hash);
ObjectPut(archive,&alen,OBJ_TREE,tree_content,tlen);

FileWrite("C:/Home/test7.hgs", archive, alen);
I64 rsize; U8 *rbuf = FileRead("C:/Home/test7.hgs", &rsize);
U16 rver; U64 rcount;
HgsReadHeader(rbuf, &rver, &rcount);

// Build the index over the reloaded object section (skip the 16-byte
// header, same convention ArchiveVerify already uses).
U8 idx_hashes[3*64];
I64 idx_offsets[3];
I64 idx_count;
IndexBuild(rbuf+16, rsize-16, idx_hashes, idx_offsets, &idx_count);
CommPrint(1,"idx_count=%d\n", idx_count);

// Look up blob1 and blob2 by their independently-computed hashes.
I64 off1, off2;
Bool found1 = IndexLookup(idx_hashes, idx_offsets, idx_count, blob1_hash, &off1);
Bool found2 = IndexLookup(idx_hashes, idx_offsets, idx_count, blob2_hash, &off2);
CommPrint(1,"found1=%d off1=%d found2=%d off2=%d\n", found1, off1, found2, off2);

// A hash that isn't in the archive must NOT be found.
U8 bogus_hash[64];
I64 bi; for (bi=0;bi<64;bi++) bogus_hash[bi]=0xEE;
I64 off_bogus;
Bool found_bogus = IndexLookup(idx_hashes, idx_offsets, idx_count, bogus_hash, &off_bogus);
CommPrint(1,"found_bogus=%d\n", found_bogus);

// The actual point: dereference a TREE ENTRY's child_hash through the
// index to fetch the real blob content, and confirm it matches.
I64 tree_pos_in_file = 16 + 5+8+64 + 5+8+64; // skip header + 2 blob records to reach the tree record
U8 reloaded_tree_type = rbuf[tree_pos_in_file+8];
U8 *reloaded_tree_content = rbuf + tree_pos_in_file + 9;

U8 found_type_a, found_hash_a[64];
TreeFindEntry(reloaded_tree_content, tlen, na, 5, &found_type_a, found_hash_a);
I64 resolved_off;
Bool resolved = IndexLookup(idx_hashes, idx_offsets, idx_count, found_hash_a, &resolved_off);
// resolved_off points at the object's [length][data...] - skip length(8)+type(1)
U8 *resolved_content = rbuf + resolved_off + 8 + 1;
Bool content_matches = (resolved_content[0]=='h' && resolved_content[1]=='e' &&
                        resolved_content[2]=='l' && resolved_content[3]=='l' &&
                        resolved_content[4]=='o');
CommPrint(1,"reloaded_tree_type=%d resolved=%d content_matches=%d\n",
          reloaded_tree_type, resolved, content_matches);

if (idx_count==3 && found1 && found2 && !found_bogus &&
    reloaded_tree_type==OBJ_TREE && resolved && content_matches)
  CommPrint(1,"PASS index_lookup_and_dereference\n");
else
  CommPrint(1,"FAIL index_lookup_and_dereference\n");
