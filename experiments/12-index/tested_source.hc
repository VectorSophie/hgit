...
// compute the tree's own content hash (tag+content), same as ObjectPut
// does internally - needed here so we can look IT up via the index too.
U8 tree_tagged[512];
...
// idx_offsets are relative to rbuf+16 (the object section start) - add
// 16 back to get true offsets into rbuf itself.
I64 off1_rel, off2_rel, off_tree_rel;
Bool found1 = IndexLookup(idx_hashes, idx_offsets, idx_count, blob1_hash, &off1_rel);
Bool found2 = IndexLookup(idx_hashes, idx_offsets, idx_count, blob2_hash, &off2_rel);
Bool found_tree = IndexLookup(idx_hashes, idx_offsets, idx_count, tree_hash, &off_tree_rel);
CommPrint(1,"found1=%d found2=%d found_tree=%d\n", found1, found2, found_tree);
...
// Resolve the tree object entirely through the index + its own stored
// length field - no hand-computed byte offsets anywhere.
I64 tree_off = 16 + off_tree_rel;
U64 tree_record_len = GetU64LE(rbuf, tree_off);   // = tlen + 1 (type tag)
U8 reloaded_tree_type = rbuf[tree_off + 8];
U8 *reloaded_tree_content = rbuf + tree_off + 9;
I64 reloaded_tree_content_len = tree_record_len - 1;

U8 found_type_a, found_hash_a[64];
Bool entry_found = TreeFindEntry(reloaded_tree_content, reloaded_tree_content_len,
                                  na, 5, &found_type_a, found_hash_a);

I64 blob_off = 16 + off1_rel;
U8 *blob_content = rbuf + blob_off + 8 + 1; // skip length field + type tag
Bool content_matches = (blob_content[0]=='h' && blob_content[1]=='e' &&
                        blob_content[2]=='l' && blob_content[3]=='l' &&
                        blob_content[4]=='o');

CommPrint(1,"reloaded_tree_type=%d entry_found=%d content_matches=%d\n",
          reloaded_tree_type, entry_found, content_matches);

if (idx_count==3 && found1 && found2 && found_tree && !found_bogus &&
    reloaded_tree_type==OBJ_TREE && entry_found && content_matches)
