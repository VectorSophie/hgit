...
// experiments/26-hgit-see-dispatch/): init/status/history/see all
// dispatch correctly against a real repository, and an unknown command
// reports cleanly.
//
// IMPORTANT HolyC quirk, confirmed by testing (see
...
//
// `offer` is still not wired - it needs a free-text message field
// (can't be naively space-split the way the others are) and a
// timestamp source. Real next work, not done here.
//
// Depends on Canon.HC, Blake2b.HC, Archive.HC, Hgs.HC, Object.HC,
// Tree.HC, Commit.HC, Index.HC, Init.HC, Head.HC, WorkDir.HC,
// Status.HC, History.HC, See.HC, Hex.HC.
...
U0 Hgit4(U8 *cmdline)
{
  ...
  if (StrCmp(cmd, "offer") == 0) {
    ...
    CommPrint(1, "offer_repo=%s offer_mask=%s offer_msg_len=%d offer_ts=%d\n",
              offer_repo_path, offer_find_mask, offer_mlen, offer_ts);
...
// Real test: offer a brand-new file through the dispatcher, with a
// multi-word commit message (the actual point of the free-text design).
U8 nf[8]; nf[0]='d';nf[1]='i';nf[2]='s';nf[3]='p';nf[4]='a';nf[5]='t';nf[6]='c';nf[7]='h';
FileWrite("C:/Home/DispOfferFile.txt", nf, 8);

Hgit4("offer C:/Home/OfferTestRepo.hgs C:/Home/DispOfferFile* dispatcher offer test message");

// verify: repo grew, head moved, and the new commit's message matches
// verbatim (spaces preserved, not just the first word)
U8 new_head[64];
HeadRead("C:/Home/OfferTestRepo.hgs", new_head);
HgitSee("C:/Home/OfferTestRepo.hgs", new_head);
