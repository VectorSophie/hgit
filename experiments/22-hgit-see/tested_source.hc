...
// See HEAD (the "second" commit): expect 1 parent, msg="second",
// 2 tree entries (OfferFileA.txt, OfferFileB.txt).
U8 head_h[64];
HeadRead("C:/Home/OfferTestRepo.hgs", head_h);
HgitSee("C:/Home/OfferTestRepo.hgs", head_h);

// Walk to the parent (the root "first" commit) and see it too: expect
// 0 parents, msg="first".
I64 rsize2;
...
I64 idx_o2[64];
I64 idx_c2;
IndexBuild(rbuf2+16, rsize2-16, idx_h2, idx_o2, &idx_c2);
I64 head_off_rel;
IndexLookup(idx_h2, idx_o2, idx_c2, head_h, &head_off_rel);
U8 *head_content = rbuf2 + 16 + head_off_rel + 9;
U8 *parent_h = CommitParentHash(head_content, 0);
HgitSee("C:/Home/OfferTestRepo.hgs", parent_h);

// A hash that doesn't exist in the repo.
U8 bogus[64];
I64 bi;
for (bi=0; bi<64; bi++) bogus[bi] = 0xAB;
HgitSee("C:/Home/OfferTestRepo.hgs", bogus);
