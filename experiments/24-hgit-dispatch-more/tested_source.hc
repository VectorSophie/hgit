...
U0 Hgit2(U8 *cmdline)
{
  ...
  if (StrCmp(cmd, "init") == 0) {
    ...
    U8 repo_path2[256];
    ExtractToken(rest, 0, repo_path2, 256);
    HgitHistory(repo_path2);
  } else {
    CommPrint(1, "DISPATCH_ERR unknown_command %s\n", cmd);
  }
}

// --- test status via dispatcher against the real repo ---
Hgit2("status C:/Home/OfferTestRepo.hgs C:/Home/OfferFile* C:/Home/");

// --- test history via dispatcher against the same real repo ---
Hgit2("history C:/Home/OfferTestRepo.hgs");

// --- unknown command still handled cleanly ---
Hgit2("frobnicate whatever");
