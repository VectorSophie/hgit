...
// hgit init: create a new, empty repository at `path` - a .HGS file
// containing only the 16-byte header (0 objects). Refuses to overwrite
// an existing file at that path (confirmed: FileRead returns NULL
// cleanly for a missing file, no exception needed - see the probe that
// ran just before this one).
...
Bool r1 = HgitInit("C:/Home/NewRepo.hgs");
Bool r2 = HgitInit("C:/Home/NewRepo.hgs"); // must fail - already exists
CommPrint(1,"r1=%d r2=%d\n", r1, r2);

I64 rsize;
U8 *rbuf = FileRead("C:/Home/NewRepo.hgs", &rsize);
U16 rver; U64 rcount;
Bool hok = HgsReadHeader(rbuf, &rver, &rcount);
CommPrint(1,"rsize=%d hok=%d version=%d count=%d\n", rsize, hok, rver, rcount);

if (r1 && !r2 && rsize==16 && hok && rver==1 && rcount==0)
  CommPrint(1,"PASS hgit_init\n");
else
  CommPrint(1,"FAIL hgit_init\n");
