...
// .HGS archive header: 16 bytes.
//   [0..3]  magic "HGS0" (0x48 0x47 0x53 0x30)
//   [4..5]  U16 format_version LE (=1)
//   [6..7]  U16 reserved LE (=0)
//   [8..15] U64 object_count LE
U0 HgsWriteHeader(U8 *buf, U16 version, U64 obj_count)
{
  buf[0]='H'; buf[1]='G'; buf[2]='S'; buf[3]='0';
  buf[4] = version & 0xFF; buf[5] = (version>>8) & 0xFF;
  buf[6] = 0; buf[7] = 0;
...
U8 hgsfile[1024];
I64 hgs_len = 16;  // reserve header space first
HgsWriteHeader(hgsfile, 1, 2);

U8 ho1[5]; ho1[0]='h';ho1[1]='g';ho1[2]='i';ho1[3]='t';ho1[4]='A';
U8 ho2[5]; ho2[0]='h';ho2[1]='g';ho2[2]='i';ho2[3]='t';ho2[4]='B';
...
U0 HgsPut(U8 *archive_buf, I64 *len, U8 *data, I64 dlen)
{
  ...
  PutU64LE(archive_buf, *len, dlen); *len += 8;
...
HgsPut(hgsfile, &hgs_len, ho1, 5);
HgsPut(hgsfile, &hgs_len, ho2, 5);

FileWrite("C:/Home/test2.hgs", hgsfile, hgs_len);
CommPrint(1,"wrote %d bytes\n", hgs_len);

I64 rsize;
U8 *rbuf = FileRead("C:/Home/test2.hgs", &rsize);
U16 rver;
U64 rcount;
Bool hok = HgsReadHeader(rbuf, &rver, &rcount);
CommPrint(1,"rsize=%d hok=%d version=%d count=%d\n", rsize, hok, rver, rcount);

I64 vtotal, vok;
ArchiveVerify(rbuf+16, rsize-16, &vtotal, &vok);
CommPrint(1,"verify total=%d ok=%d\n", vtotal, vok);

if (hok && rver==1 && rcount==2 && vtotal==2 && vok==2)
  CommPrint(1,"PASS hgs_header_roundtrip\n");
else
  CommPrint(1,"FAIL hgs_header_roundtrip\n");
