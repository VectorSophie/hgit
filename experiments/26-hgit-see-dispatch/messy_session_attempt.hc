...
U8 HexDigit(U8 c)
{
  ...
  if (c>='A' && c<='F') return c-'A'+10;
  return 0xFF; // sentinel for invalid - caller should treat as error
}

// hex must be exactly 128 chars (no NUL check here - caller's job).
// Returns FALSE if any character isn't a valid hex digit.
...
// --- round-trip test on a real object hash (HEAD of the real repo) ---
U8 head_h[64];
HeadRead("C:/Home/OfferTestRepo.hgs", head_h);

U8 hex[129];
HashToHex(head_h, hex);
CommPrint(1, "hex=%s\n", hex);

U8 back[64];
Bool ok = HexToHash(hex, back);
Bool match = TRUE;
I64 i;
for (i=0; i<64; i++) if (back[i]!=head_h[i]) match=FALSE;
CommPrint(1, "parse_ok=%d roundtrip_match=%d\n", ok, match);

// --- invalid hex must be rejected, not silently accepted ---
U8 bad_hex[129];
for (i=0; i<128; i++) bad_hex[i] = '0';
bad_hex[5] = 'z'; // not a valid hex digit
bad_hex[128] = 0;
U8 dummy[64];
Bool bad_ok = HexToHash(bad_hex, dummy);
CommPrint(1, "bad_hex_rejected=%d (expect 0)\n", !bad_ok);

// --- use the parsed hash to actually call HgitSee, proving the whole
// hex-string-driven path works end to end, not just byte round-trip ---
HgitSee("C:/Home/OfferTestRepo.hgs", back);

if (ok && match && !bad_ok)
  CommPrint(1, "PASS hex_hash_parsing\n");
else
  CommPrint(1, "FAIL hex_hash_parsing\n");
U0 Hgit3(U8 *cmdline)
{
  I64 i = 0;
  while (cmdline[i] && cmdline[i] != ' ') i++;
  ...
  if (StrCmp(cmd, "see") == 0) {
    ...
    I64 p = 0;
    p = ExtractToken(rest, p, see_repo_path, 256);
    p = ExtractToken(rest, p, see_hex, 129);
    U8 see_hash[64];
...
U8 dh[64];
HeadRead("C:/Home/OfferTestRepo.hgs", dh);
U8 dhex[129];
HashToHex(dh, dhex);

U8 cmdbuf[300];
I64 k = 0;
U8 *pfx = "see C:/Home/OfferTestRepo.hgs ";
while (pfx[k]) { cmdbuf[k]=pfx[k]; k++; }
I64 m;
for (m=0; m<128; m++) cmdbuf[k+m]=dhex[m];
cmdbuf[k+128]=0;

Hgit3(cmdbuf);

// bad hash via dispatcher
Hgit3("see C:/Home/OfferTestRepo.hgs notarealhash");
