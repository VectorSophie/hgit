...
U8 test_head_hash[64];
HeadRead("C:/Home/OfferTestRepo.hgs", test_head_hash);
U8 test_hex[129];
HashToHex(test_head_hash, test_hex);

U8 test_cmdbuf[300];
I64 tc_k = 0;
U8 *test_pfx = "see C:/Home/OfferTestRepo.hgs ";
while (test_pfx[tc_k]) { test_cmdbuf[tc_k]=test_pfx[tc_k]; tc_k++; }
I64 tc_m;
for (tc_m=0; tc_m<128; tc_m++) test_cmdbuf[tc_k+tc_m]=test_hex[tc_m];
test_cmdbuf[tc_k+128]=0;

CommPrint(1, "--- see via dispatcher, real HEAD hash ---\n");
Hgit(test_cmdbuf);

CommPrint(1, "--- see via dispatcher, garbage hash ---\n");
Hgit("see C:/Home/OfferTestRepo.hgs notarealhash");

CommPrint(1, "--- unknown command still handled ---\n");
Hgit("frobnicate whatever");
