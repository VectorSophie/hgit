...
U0 HgitHistory(U8 *repo_path)
{
  ...
  while (has_cur) {
    I64 off;
    Bool found = IndexLookup(idx_hashes, idx_offsets, idx_count, cur_hash, &off);
    if (!found) { CommPrint(1, "HISTORY_ERR broken_chain\n"); return; }
    U8 obj_type = rbuf[off+8];
    U8 *content = rbuf + off + 9;
    if (obj_type != OBJ_COMMIT) { CommPrint(1, "HISTORY_ERR not_a_commit\n"); return; }
...
// Reuse the real repo hgit offer already built and left on disk from
// the previous session (two offerings: "first" then "second").
HgitHistory("C:/Home/OfferTestRepo.hgs");

// Also check the empty case on a never-offered repo.
HgitInit("C:/Home/HistoryEmptyRepo.hgs");
HgitHistory("C:/Home/HistoryEmptyRepo.hgs");
