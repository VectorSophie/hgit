...
U0 HgitStatus3(U8 *repo_path, U8 *find_mask, U8 *dir_prefix)
{
  I64 rsize;
  ...
  // pass 1: new/modified/unchanged (as probe 20)
  ...
  // pass 2: deleted - walk every tree entry, check it still exists on disk
...
// Delete OfferFileB.txt (previously UNCHANGED per probe 20) and check
// it's now reported deleted, while A/C are unaffected by this change.
Del("C:/Home/OfferFileB.txt", FALSE, FALSE, FALSE); // real API from Kernel/BlkDev/DskCopy.HC, not "FileDel"
HgitStatus3("C:/Home/OfferTestRepo.hgs", "C:/Home/OfferFile*", "C:/Home/");
