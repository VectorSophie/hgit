...
// The "current offering" pointer: a small sidecar file alongside the
// repo (`<repo_path>.head`) holding exactly 64 bytes - the hash of the
// latest commit. Absent = no offerings yet (same convention HgitInit/
// HgitStatus already use: FileRead returning NULL means "doesn't exist",
// not an error).
...
// --- test ---
HgitInit("C:/Home/HeadTestRepo.hgs");

U8 h1[64];
Bool found_before = HeadRead("C:/Home/HeadTestRepo.hgs", h1);
CommPrint(1,"found_before=%d (expect 0, no offerings yet)\n", found_before);

U8 fake_commit_hash[64];
I64 i;
for (i=0;i<64;i++) fake_commit_hash[i] = i+1;
HeadWrite("C:/Home/HeadTestRepo.hgs", fake_commit_hash);

U8 h2[64];
Bool found_after = HeadRead("C:/Home/HeadTestRepo.hgs", h2);
Bool match = TRUE;
for (i=0;i<64;i++) if (h2[i]!=fake_commit_hash[i]) match=FALSE;
CommPrint(1,"found_after=%d match=%d\n", found_after, match);

// update head to a second value, confirm it actually changes (not
// stuck on the first write)
U8 fake_commit2[64];
for (i=0;i<64;i++) fake_commit2[i] = 200-i;
HeadWrite("C:/Home/HeadTestRepo.hgs", fake_commit2);
U8 h3[64];
HeadRead("C:/Home/HeadTestRepo.hgs", h3);
Bool match2 = TRUE;
for (i=0;i<64;i++) if (h3[i]!=fake_commit2[i]) match2=FALSE;
CommPrint(1,"match2=%d\n", match2);

if (!found_before && found_after && match && match2)
  CommPrint(1,"PASS head_pointer\n");
else
  CommPrint(1,"FAIL head_pointer\n");
