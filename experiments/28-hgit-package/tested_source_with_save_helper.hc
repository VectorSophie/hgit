...
U0 HgitSee(U8 *repo_path, U8 *target_hash)
{
  ...
  for (te=0; te<tcount; te++) {
    ...
    tpos += 64;
    CommPrint(1, "  entry type=%d name=", ttype);
...
U8 *hgit_pkg_saved = MAlloc(54379);
U0 SaveHgitPackage()
{
  I64 hgit_pkg_i;
  for (hgit_pkg_i=0; hgit_pkg_i<54379; hgit_pkg_i++) hgit_pkg_saved[hgit_pkg_i] = Db[hgit_pkg_i];
  FileWrite("C:/Home/HgitAll.HC", hgit_pkg_saved, 54379);
  CommPrint(1, "SAVED\n");
}
SaveHgitPackage();
