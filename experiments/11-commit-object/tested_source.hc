...
// Commit content format (this IS the `data`/`dlen` passed to ObjectPut
// with type=OBJ_COMMIT):
//   64 bytes  tree_hash
//   U8        parent_count
...
// --- build: 2 blobs -> 1 tree -> root commit -> child commit (1 parent) ---
U8 archive[8192];
I64 alen = 16;
HgsWriteHeader(archive, 1, 5); // 2 blobs + 1 tree + 2 commits
...
// compute tree's own content-hash the same way ObjectPut would (tag + content)
U8 tree_tagged[512];
tree_tagged[0]=OBJ_TREE;
I64 ti; for (ti=0; ti<tlen; ti++) tree_tagged[1+ti]=tree_content[ti];
U8 tree_hash[64];
B2Hash512Any(tree_tagged, tlen+1, tree_hash);
ObjectPut(archive,&alen,OBJ_TREE,tree_content,tlen);

// root commit: 0 parents
U8 root_msg[13]; root_msg[0]='r';root_msg[1]='o';root_msg[2]='o';root_msg[3]='t';root_msg[4]=' ';
root_msg[5]='c';root_msg[6]='o';root_msg[7]='m';root_msg[8]='m';root_msg[9]='i';root_msg[10]='t';root_msg[11]='!';root_msg[12]=0;
U8 commit1_content[256];
I64 c1len = 0;
CommitEncode(commit1_content, &c1len, tree_hash, NULL, 0, 1000000, root_msg, 13);
U8 commit1_tagged[256];
commit1_tagged[0]=OBJ_COMMIT;
I64 ci; for (ci=0; ci<c1len; ci++) commit1_tagged[1+ci]=commit1_content[ci];
U8 commit1_hash[64];
B2Hash512Any(commit1_tagged, c1len+1, commit1_hash);
ObjectPut(archive, &alen, OBJ_COMMIT, commit1_content, c1len);

// child commit: 1 parent (commit1)
U8 child_msg[6]; child_msg[0]='c';child_msg[1]='h';child_msg[2]='i';child_msg[3]='l';child_msg[4]='d';child_msg[5]=0;
U8 commit2_content[256];
I64 c2len = 0;
CommitEncode(commit2_content, &c2len, tree_hash, commit1_hash, 1, 2000000, child_msg, 6);
I64 commit2_off = alen;
ObjectPut(archive, &alen, OBJ_COMMIT, commit2_content, c2len);

CommPrint(1,"alen=%d c1len=%d c2len=%d\n", alen, c1len, c2len);

FileWrite("C:/Home/test6.hgs", archive, alen);
I64 rsize; U8 *rbuf = FileRead("C:/Home/test6.hgs", &rsize);
U16 rver; U64 rcount;
...
// decode commit2 back out of the reloaded buffer
I64 c2_pos_in_file = commit2_off + 8; // skip length field
U8 reloaded_type2 = rbuf[c2_pos_in_file];
U8 *c2_content = rbuf + c2_pos_in_file + 1;

Bool tree_hash_match = TRUE;
I64 k;
for (k=0;k<64;k++) if (CommitTreeHash(c2_content)[k]!=tree_hash[k]) tree_hash_match=FALSE;
U8 pcount = CommitParentCount(c2_content);
Bool parent_hash_match = TRUE;
for (k=0;k<64;k++) if (CommitParentHash(c2_content,0)[k]!=commit1_hash[k]) parent_hash_match=FALSE;
U64 ts = CommitTimestamp(c2_content);
U32 mlen = CommitMessageLen(c2_content);
U8 *msg = CommitMessage(c2_content);
Bool msg_match = (mlen==6 && msg[0]=='c' && msg[1]=='h' && msg[2]=='i' && msg[3]=='l' && msg[4]=='d');

CommPrint(1,"reloaded_type2=%d tree_hash_match=%d pcount=%d parent_hash_match=%d ts=%d mlen=%d msg_match=%d\n",
          reloaded_type2, tree_hash_match, pcount, parent_hash_match, ts, mlen, msg_match);

if (hok && rcount==5 && vt==5 && vok==5 && reloaded_type2==OBJ_COMMIT &&
    tree_hash_match && pcount==1 && parent_hash_match && ts==2000000 && msg_match)
  CommPrint(1,"PASS commit_object_roundtrip\n");
else
  CommPrint(1,"FAIL commit_object_roundtrip\n");
