...
| [04-vcs-comparison.md](04-vcs-comparison.md) | 🟡 | jj's operation log, Fossil's delta format, and Sapling's undo/absorb model all read and compared; Pijul/GitButler/Mercurial still unfetched |
...
| [06-storage-hashing-compression.md](06-storage-hashing-compression.md) | 🟡 | Canonical encoding, full BLAKE2b-512, a tiny append/read/verify object archive, **and a versioned `.HGS` header format** (see `FORMAT.md`) all passing in native HolyC on real TempleOS; Fossil delta format prototyped and partially verified (ADR 0008) - byte-level mechanics correct, a real reliability gap left open; content-defined chunking still not started |
